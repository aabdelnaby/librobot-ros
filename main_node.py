...
class LibraryHelp:
    ...
    def __init__(self, landmark_list, books_list, default_list, linear_speed,
                 angular_speed):
        ...
        self.robot_model = rospy.get_param('~robot_name', 'turtlebot2')
    ...
    def default_behavior(self):
        ...
        for i in self.default_list:
            ...
            # Rotates a bit, then uses a loop to get back to old yaw
            self.t.angular.z = self.angular_speed
            self.vel_pub.publish(self.t)
            time.sleep(1)

            diff = self.get_a_distance(self.odo_yaw, self.current_yaw)
            # Uses this to spot any books, the clbk_marker function in this time will
            # add all the books it sees to the seen_books list 
            while(diff >= np.radians(20)):
                diff = self.get_a_distance(self.odo_yaw, self.current_yaw)
                self.vel_pub.publish(self.t)
                self.rate.sleep()
...
if __name__ == '__main__':
    ...
    shelves = []
    books = []
    defaults = [(0, 2), (0, 4), (0, 6)]
    linear_speed = 0.4
    rotation_speed = 0.4
    node = LibraryHelp(shelves, books, 
                       defaults, linear_speed, rotation_speed)
